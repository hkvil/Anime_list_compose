...
object WaifuRepository {

        fun getWaifuByName(name:String,context: Context):Waifu?{
            val waifuList = getWaifuArrayData(context)
            var waifu: Waifu? = null
            for(i in waifuList){
                if(i.waifuName == name){
                    waifu = i
                }
        ...
        fun searchWaifus(query: String,context:Context): List<Waifu>{
            return getWaifuArrayData(context).filter {
                it.waifuName.contains(query,ignoreCase = true)
            }
        }


        fun getWaifuArrayData(context:Context): ArrayList<Waifu> {
            val context: Context = context.applicationContext
            val waifuArray = ArrayList<Waifu>()
            val waifuName = context.resources.getStringArray(R.array.data_name)
            val waifuVoiceActor = context.resources.getStringArray(R.array.data_voiced_by)
            val waifuAge = context.resources.getStringArray(R.array.data_age)
            val waifuAnimeTitle = context.resources.getStringArray(R.array.data_anime_title)
            val waifuBirthday = context.resources.getStringArray(R.array.data_birth_day)
            val waifuPhoto = context.resources.obtainTypedArray(R.array.data_waifu_photo)
            val waifuVoiceActorPhoto = context.resources.obtainTypedArray(R.array.data_voice_photo)
        ...
        fun getWaifuIntro(index: Int): String {//I put here because its produced error when i put on the strings.xm
            val waifuIntro = arrayListOf(
